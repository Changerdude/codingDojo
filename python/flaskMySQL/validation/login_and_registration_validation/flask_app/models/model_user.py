...
from flask_app import DATABASE,TEN_YEARS_AGO
...
class User:
    ...
    @classmethod
    def create(cls,data):
        query = "INSERT INTO users ( first_name , last_name , email , pw_hash , birthday) VALUES ( %(first_name)s , %(last_name)s , %(email)s , %(pw_hash)s , %(birthday)s);"
    ...
    def validate_registration(cls,data):
        ...
        if len(data["birthday"]) < 1:
            flash("Need to select birthday", "err_birthday_create")
            is_valid = False
        elif data["birthday"] < TEN_YEARS_AGO:
            flash("Must be ten years old", "err_birthday_create")
            is_valid = False
        if len(data['pw']) < 8:
